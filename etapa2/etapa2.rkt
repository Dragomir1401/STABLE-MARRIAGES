...
(define (get-women wpref)
  (foldr (λ (L acc) (cons (car L) acc)) '() wpref))
...
(define (get-partner engagements person)
  (define (get-first-partner engagements person)
    (find-first (λ (L) (equal? (car L) person)) engagements))
...
(define (update-engagements engagements p1 p2)
  (change-first (λ (l) (equal? (car l) p1)) engagements (cons p1 p2)))
